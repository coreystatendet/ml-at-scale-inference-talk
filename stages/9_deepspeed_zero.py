...
# Even though hf_ds_config is not passed anywhere, it must remain in scope while
# `from_pretrained` is called.  It sets a global variable with a weakref that's used behind the
# scenes, and if it's memory-collected before the call the weakref will be cleared.
hf_ds_config = transformers.deepspeed.HfDeepSpeedConfig(hparams["deepspeed_config"])
tokenizer = transformers.AutoTokenizer.from_pretrained(hparams["model_name"], padding_side="left")
if not (tokenizer.pad_token):
    tokenizer.pad_token = tokenizer.eos_token
model = transformers.AutoModelForCausalLM.from_pretrained(
    hparams["model_name"], torch_dtype=getattr(torch, hparams["dtype"])
).to("cuda")
ds_engine = deepspeed.initialize(model=model, config_params=hparams["deepspeed_config"])[0]
...
with det.core.init(distributed=det.core.DistributedContext.from_deepspeed()) as core_context:
    ...
    generate(tokenizer, model, batch_size=hparams["batch_size"])

    # Timed run.
    timer = CumulativeTimer()
    timer.start()
    total_tokens = core_context.distributed.gather(
        generate(tokenizer, model, batch_size=hparams["batch_size"])
    )
    timer.stop()
